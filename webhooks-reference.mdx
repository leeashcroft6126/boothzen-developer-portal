...
<AccordionGroup>
  <Accordion title="Booking events (3)">
...
The exact shape of `data.object` for each resource is part of the OpenAPI contract. Browse it in the [API Reference](/api-reference) — the webhook resource shape matches the corresponding `GET /api/v1/{resource}/{id}` response.
